...
#ifndef PYABORIA_PYTHON_H_
#define PYABORIA_PYTHON_H_
...
template <typename T, unsigned int D> struct VectFromPythonList {

  VectFromPythonList() {
    boost::python::converter::registry::push_back(
        &convertible, &construct, boost::python::type_id<Vector<T, D>>());
  }

  static void *convertible(PyObject *obj_ptr) {
    if (!PyList_Check(obj_ptr))
      return 0;
    if (PyList_Size(obj_ptr) != D)
      return 0;
    return obj_ptr;
  }
  ...
  construct(PyObject *obj_ptr,
    ...
    for (int i = 0; i < D; ++i) {
      (*static_cast<Vector<T, D> *>(storage))[i] =
          extract<T>(PyList_GetItem(obj_ptr, i));
    }
...
template <typename T, unsigned int D> struct VectToPython {
...
typename ParticlesType::reference getitem_particles(ParticlesType &p,
                                                    int index) {
...
template <typename ParticlesType>
void setitem_particles_from_value(
    ParticlesType &p, int index,
    const typename ParticlesType::value_type &val) {
  if (index < 0 || index >= p.size()) {
    PyErr_SetString(PyExc_IndexError, "index out of range");
    throw boost::python::error_already_set();
    ;
...
template <typename ParticlesType>
void setitem_particles_from_reference(
    ParticlesType &p, int index, const typename ParticlesType::reference &ref) {
  if (index < 0 || index >= p.size()) {
    PyErr_SetString(PyExc_IndexError, "index out of range");
    throw boost::python::error_already_set();
    ;
...
template <typename ParticlesType>
void particles_push_back(ParticlesType &p,
                         const typename ParticlesType::value_type &val) {
  p.push_back(val);
}

template <class T> struct vtkSmartPointer_to_python {
...
template <class T> struct vtk_to_python {
